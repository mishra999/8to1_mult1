...
for i in range(M1):
    corr1[i] = signal.correlate(res_65[i,0]/np.max(res_65[i,0,80:200]), res_65[i,1]/np.max(res_65[i,1,80:200]), mode='full') / len(res_65[i,1])
...
#    corr1[i] = np.lib.pad(cor1, (500,500), 'constant', constant_values=(0., 0.))
    corr2[i] = signal.correlate(res_65[i,1]/np.max(res_65[i,1,80:200]), res_65[i,1]/np.max(res_65[i,1,80:200]), mode='full') / len(res_65[i,1])
