...
#                print(vch1[ch_ind,cvc])
        ...
        self.time_samples = np.zeros((4,1024,1024))#channel,trigger cell, timebins
    ...
    def imp_response(self):
        ...
        res = {'50': 1, '55': 1, '70': 1, '65': 1}
#a2 = np.zeros((10000,10000))
        chchch = [res[self.resonator],0] 
        for i1 in range(2):
            cct =0
...
#    plt.tight_layout()
plt.figure()
plt.plot(f00[0:1000],np.abs(np.fft.fft(cor22))[0:1000])    
plt.plot(f00[0:1000],np.abs(np.fft.fft(cor11))[0:1000])    

#second
    qq=drs_fdm_parser(r'C:\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\impres509.dat','50',1,2,23000)
...
#    plt.plot(f0,np.abs(np.fft.fft(cor1)))
    ...
    qq=drs_fdm_parser(r'C:\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\impres559.dat','50',1,2,23000)
    ...
    qq=drs_fdm_parser(r'C:\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\impres709.dat','60',2,2,23000)
...
#    
    ...
    h65_t1, cor22, cor11 = qq.imp_response()
    h651 = np.abs(np.fft.fft(h65_t1[0:1000]))
    plt.figure()
    plt.plot(f0[0:500],np.abs(h651[0:1000])[0:500])
    plt.xlabel('frequency (MHz)')
    plt.ylabel('amplitude (arb. units)')
    plt.tight_layout()
...
#    plt.plot(h501)
    ...
    qq=drs_fdm_parser(r'C:\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\new\impres55.dat','50',1,2,23000)
    h55_t, cor22, cor11 = qq.imp_response()
    h55 = np.abs(np.fft.fft(h55_t[0:1000]))
    plt.figure()
