...
vch1 = np.zeros((2,4,500000,1024))
tcell = np.zeros((2,500000))
#read board header
with open(r'C:\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\8to1mult_999.dat', 'rb') as f:
...
#            thres = 0.3
        cvc +=1
        if cvc >499990: #number of events to read (n-1)
            break
...
#plt.tight_layout()
plt.figure()
plt.plot(f0[0:500],np.abs(np.fft.fft(a00[1,410]))[0:500])
...
#    res_temp85 = []
    ...
    if 50 + np.argmax(d1[50:85])== 67 or 50 + np.argmax(d1[50:85])== 68 or 50 + np.argmax(d1[50:85])== 69 or 50 + np.argmax(d1[50:85])== 66 or 50 + np.argmax(d1[10:500])== 65:
...
for i22 in range(5000,len(res_65)):
